...
const HabitEditForm: React.FC<{
  ...
  const [warningHabitNameEmpty, setWarningHabitNameEmpty] =
    useState<boolean>(false);
  const [warningHabitNameExists, setWarningHabitNameExists] =
    useState<boolean>(false);
  ...
  const handleNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (newHabitName !== editHabitName) {
      ...
      setWarningHabitNameExists(
        habitExists.includes(newHabitName.toLowerCase())
      );
    } else {
  ...
  return (
    <form onSubmit={handleEdit} className='habit-edit-form'>
      ...
      {warningChooseDay && (
        <p className='warning-text'>You haven't chosen any day</p>
      )}

      <button
        type='submit'
        disabled={warningHabitNameExists || warningChooseDay}
      >
        Edit Habit
      </button>
    </form>
