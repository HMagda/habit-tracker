    ...
    getToken(getAccessTokenSilently).then((token) => {
      fetch(baseUrl + `/habits/today/${id}/complete/${isDone}`, {
        ...
        .then((data) => {
          ...
          const changedTodayHabit: HabitForToday =
            fetchedHabitsForTodayArr.find(
              (habit: HabitForToday) => habit.id === id
            );
          const completionStatus =
            changedTodayHabit?.completed != null
              ? changedTodayHabit.completed
              : isDone;
            ...
            habit.id === id
              ? {
                  ...
                  days: habit.days.map((day: HabitDay) =>
                    day.dayOfWeek === data.todayIndex
                      ? {...day, completed: completionStatus}
                      : day
                  ),
                }
              : habit
